...
/** 카테고리별 간단 프로시저럴 아이콘. (자가발광 emissive) */
...
export function CategoryNode({ category, position, selected, dimmed = false, onSelect }: Props) {
  ...
  const glow = active ? 1.3 : dimmed ? 0.5 : 1;

  // 호버/선택 시 부드러운 스케일만 — 부유·회전·펄스 없음(가지런/깜빡임 제거).
  useFrame(() => {
    const target = active ? 1.28 : dimmed ? 0.92 : 1;
    if (groupRef.current) {
  ...
  return (
    <group position={position}>
      ...
      <Html center position={[0, 1.55, 0]} distanceFactor={11} zIndexRange={[20, 0]} occlude={false}>
        <div
          style={{
            color: "#f1f5f9",
            ...
            padding: "3px 9px",
            borderRadius: 8,
            background: active ? "rgba(2,6,12,0.85)" : "rgba(2,6,12,0.62)",
