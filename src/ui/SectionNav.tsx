/** 놀리지 아틀라스 대분류(섹션) 스위처 — 산업 / 기초이론. 화면 최상단 중앙. */
...
export function SectionNav({ section, onChange }: { section: Section; onChange: (s: Section) => void }) {
  return (
    <div style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 40 }}>
      <div className="glass-strong rounded-2xl" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 8px" }}>
        <div style={{ display: "flex", flexDirection: "column", lineHeight: 1.1, padding: "0 10px 0 6px" }}>
          <span style={{ fontSize: 8.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "#64748b" }}>Knowledge</span>
          <span style={{ fontSize: 13, fontWeight: 800, color: "#f1f5f9", letterSpacing: "0.02em" }}>아틀라스</span>
        </div>
          ...
          return (
            ...
            >
              ...
              <span style={{ display: "flex", flexDirection: "column", lineHeight: 1.15, textAlign: "left" }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: active ? "#fff" : "#cbd5e1" }}>{s.label}</span>
                <span style={{ fontSize: 9.5, color: active ? "#bae6fd" : "#64748b" }}>{s.sub}</span>
              </span>
            </button>
          );
        })}
